...
public class DtlsConfigurationTest {
    ...
    public void testConfiguration() {
        assertEquals(1.0, dtlsConfiguration.getMinVersion(), 0);
        assertEquals(1.2, dtlsConfiguration.getMaxVersion(), 0);
        assertEquals(4, dtlsConfiguration.getCipherSuites().length);
        final DtlsConfiguration.CertificateConfiguration certificate = dtlsConfiguration.getCertificate();
        assertEquals("../conf/dtls/x509-server-ecdsa.pem", certificate.getPath());
        assertEquals("../conf/dtls/x509-server-key-ecdsa.pem", certificate.getKey());
